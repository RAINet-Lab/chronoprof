...
#os.environ["CUDA_VISIBLE_DEVICES"] = "2" #select here your visible gpu device
dev = torch.device("cuda" if torch.cuda.is_available() else "cpu")
from models import LSTM, DLinear, PatchTST, Linear, TSMixer
...
#-------------------------------------------------------------------------------------------------------------------------
dataset_dict={'users':load_users,'euma':load_euma,'ili':load_basic,'etth2':load_basic,'Vtraffic':load_basic,'synthetic':load_basic,'cperiod':load_basic} #add here your loading function for your dataset
model_dict={'lstm': LSTM.Model,'patchtst': PatchTST.Model,'dlinear': DLinear.Model,'linear':Linear.Model,'tsmixer':TSMixer.Model} #add here your model
...
train_loader = data.DataLoader(data.TensorDataset(X_train[:int(X_train.shape[0]*0.8)], y_train[:int(y_train.shape[0]*0.8)].detach()), shuffle=True, batch_size=args.batch_size)
val_loader=data.DataLoader(data.TensorDataset(X_train[int(X_train.shape[0]*0.8):], y_train[int(y_train.shape[0]*0.8):].detach()), shuffle=False, batch_size=args.batch_size)
...
if args.calculate_shap == 'True':
    ...
    if X_train.shape[0]>args.background_samples:
        size=args.background_samples
    else:
        size=X_train.shape[0]
    
    train_array=X_train.reshape(X_train.shape[0],X_train.shape[1]*X_train.shape[2]).cpu().detach().numpy()[np.random.choice(X_train.shape[0], size=size, replace=False)]
    test_array=X_test.reshape(X_test.shape[0],X_test.shape[1]*X_test.shape[2]).cpu().detach().numpy()
