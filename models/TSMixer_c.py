...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.eps = configs.tsmixer_eps
        self.input_length=configs.seq_len
        self.forecast_length=configs.pred_len
        self.no_feats=configs.enc_in
        self.feat_mixing_hidden_channels=configs.feat_mixing_hidden_channels
        self.no_mixer_layers=configs.no_mixer_layers
        self.dropout=configs.dropout
        self.softmax=torch.nn.Softmax(dim=-1)

        # Scale and shift params to learn
        self.scale = nn.Parameter(torch.ones(self.no_feats))
        self.shift = nn.Parameter(torch.zeros(self.no_feats))

        # ts mixer layers
        self.ts = TSMixerModelExclRIN(
            input_length=self.input_length, 
            forecast_length=self.forecast_length, 
            no_feats=self.no_feats, 
            feat_mixing_hidden_channels=self.feat_mixing_hidden_channels,
            no_mixer_layers=self.no_mixer_layers,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x=self.softmax(x.permute(0,2,1)) #B,C,L
        return x