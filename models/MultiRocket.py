...
def fit(X, num_features=50_000, max_dilations_per_kernel=32):
...
@njit(
    "float32[:,:](float32[:,:],float32[:,:],Tuple((int32[:],int32[:],float32[:])),Tuple((int32[:],int32[:],float32[:])),int64)",
...
class Model(nn.Module):
    def __init__(self,configs):
        ...
        self.num_features = 50000 / 2  # 1 per transformation
        self.num_kernels = int(self.num_features / self.n_features_per_kernel)
        self.pred_len = configs.pred_len
        self.linear= nn.Linear(49728,self.pred_len)
    ...
    def forward(self, x):
        xx = torch.diff(x.squeeze(), 1)
        x_transform = transform(
            x.detach().cpu()[:,:,0].numpy().astype(np.float32), xx.detach().cpu().numpy().astype(np.float32),
            self.base_parameters, self.diff1_parameters,
            self.n_features_per_kernel
        )
        x_transform = torch.tensor(np.nan_to_num(x_transform),device=x.device.type).float()

        return self.linear(x_transform).unsqueeze(dim=-1)